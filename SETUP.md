...
Что не переезжает само — это разговор с Claude: он живёт на том компьютере,
где запущен Claude Code. Состояние дел на такой случай собрано
в [HANDOVER.md](HANDOVER.md) — на новой машине начните с него.
...
Ключ от сервера не пересылайте сообщениями и не копируйте вовсе: **заведите
на новой машине свой.** Закрытая половина тогда никуда не едет, а открытую
можно передавать чем угодно — она не секрет.

```bash
ssh-keygen -t ed25519 -f "$HOME/.ssh/notarybot_vps" -C "домашний компьютер"
```

Дальше содержимое `notarybot_vps.pub` дописать в `/root/.ssh/authorized_keys`
на сервере. Двумя путями:

1. **Со старой машины**, пока доступ с неё ещё есть, — одной командой.
2. **Через консоль в панели хостинга**, если старой машины уже нет.

Второй путь стоит проверить заранее. Проверено 2026-08-23: пущен всего один
ключ, вход по паролю в SSH выключен, пароль root установлен. То есть консоль
панели — единственная запасная дверь, и убедиться, что она открывается,
надо **до** того, как единственный ключ останется на выключенной машине.
Она работает не через SSH, поэтому запрет пароля на неё не действует.

Ключи машин, которыми больше не пользуетесь, из `authorized_keys` убирайте.

Телефон в роли флешки годится не всякий: iPhone отдаёт Windows только
фотоплёнку и только на чтение, записать в него файл нельзя.
